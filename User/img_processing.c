...
float lx[50];
float rx[50];
int angle_pos_l=0,angle_pos_r=0;
float cross_exit_l,cross_exit_r;
...
void searchline_OV7620(void)
{
  ...
  for (CurL = 0; CurL<row_num; CurL++){
          p = imgadd + CurL*col_num;                                                            //指向当前行
          if (CurL>0){                                                                        //确定左右线扫描起点  
                  LastL = CurL - 1;                                                           //Lx[]=col_num表示没有找到左黑线
                  while (LastL>0 && CurL - LastL<5 && Lx[LastL] == col_num)LastL--;
                  if (Lx[LastL] != col_num)                                                   //在本行前五行找到上一个线中心
                  {
