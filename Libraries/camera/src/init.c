...
void init(void){
    ...
    FTM_QD_QuickInit(FTM1_QD_PHA_PB00_PHB_PB01,kFTM_QD_NormalPolarity,kQD_PHABEncoding);//FTM1 PTB00 PTB01 正交解码
    
    init_ov7620();//摄像头初始化
    init_steer();//舵机初始化
    init_nixietube(); //数码管初始化
    init_motor(); //电机初始化
    init_uart();  //uart初始化
...
void init_steer(void){
  ...
  FTM_PWM_QuickInit(FTM2_CH0_PB18,kPWM_EdgeAligned,250,10);
   //FTM_PWM_InvertPolarity(HW_FTM2,HW_FTM_CH0,kFTM_PWM_LowTrue);              // 50  705/10000      250 3600/10000
  
}

void init_motor(void)
{
  FTM_PWM_QuickInit(FTM0_CH4_PD04,kPWM_EdgeAligned,3000,1000);
  FTM_PWM_QuickInit(FTM0_CH7_PD07,kPWM_EdgeAligned,3000,1000);
}

void init_nixietube(void)
{
